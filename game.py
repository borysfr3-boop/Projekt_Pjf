...
from map_data import SCREEN_W, SCREEN_H, WAYPOINTS, draw_background, draw_path, draw_base, is_on_path
...
class Game:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("Tower Defense: Obrona Stacji (MVP)")
    ...
    def reset_game(self):
        ...
        self.wave_manager = WaveManager(WAYPOINTS)
    ...
    def is_too_close_to_other_tower(self, pos):
        px, py = pos
        for t in self.towers:
            if (t.pos.x - px)**2 + (t.pos.y - py)**2 < (32)**2:
                return True
        return False
    ...
    def try_build_tower(self, pos, mode):
        if mode == "Laser":
            ...
            if self.credits >= cost:
                self.credits -= cost
                self.towers.append(LaserTower(pos))
        elif mode == "Cannon":
            cost = CannonTower.COST
            if self.credits >= cost:
                self.credits -= cost
                self.towers.append(CannonTower(pos))
        elif mode == "Slow":
            cost = SlowTower.COST
            if self.credits >= cost:
                self.credits -= cost
                self.towers.append(SlowTower(pos))
    ...
    def update(self, dt):
        ...
        self.enemies = [e for e in self.enemies if e.alive or e.reached_base]  # reached_base zostanie "skasowany" po -HP
        self.enemies = [e for e in self.enemies if e.alive]  # teraz tylko żywe
    ...
    def draw(self):
        ...
        draw_base(self.screen, self.base_rect)
    ...
    def draw_menu(self):
        ...
        sub = self.font.render("Obrona stacji kosmicznej (MVP)", True, (200, 200, 230))
        ...
        tip = self.small_font.render("Sterowanie: 1/2/3 wybór wieży, klik - buduj, U - ulepsz, Spacja - fala", True, (170, 170, 210))
