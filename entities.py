...
class Enemy:
    ...
    def update(self, dt: float):
        ...
        # progress - proste (idx + "odległość do następnego" w drugą stronę)
    ...
    def draw(self, screen):
        ...
        # kolor zależny od slow
        body = (220, 70, 70) if self.slow_factor >= 1.0 else (120, 170, 255)
        pygame.draw.circle(screen, body, (int(self.pos.x), int(self.pos.y)), self.radius)
        ...
        pygame.draw.rect(screen, (30, 30, 40), (x, y, w, h), border_radius=3)
        fill = int(w * max(0.0, self.hp / self.hp_max))
        pygame.draw.rect(screen, (80, 220, 120), (x, y, fill, h), border_radius=3)
        pygame.draw.rect(screen, (150, 150, 200), (x, y, w, h), 1, border_radius=3)
...
class Tower:
    ...
    def pick_target(self, enemies):
        # wybierz "najdalej" na trasie w zasięgu
        best = None
        best_prog = -1e9
        for e in enemies:
            if not e.alive:
    ...
    def draw(self, screen, selected=False):
        col = (160, 160, 200) if not selected else (255, 255, 255)
        pygame.draw.circle(screen, (60, 60, 90), (int(self.pos.x), int(self.pos.y)), self.radius)
        pygame.draw.circle(screen, col, (int(self.pos.x), int(self.pos.y)), self.radius, 2)
...
class LaserTower(Tower):
    ...
    def draw(self, screen, selected=False):
        ...
        pygame.draw.circle(screen, (120, 200, 255), (int(self.pos.x), int(self.pos.y)), 5)
...
class CannonTower(Tower):
    ...
    def draw(self, screen, selected=False):
        super().draw(screen, selected)
        pygame.draw.rect(screen, (255, 210, 130),
                         (int(self.pos.x)-6, int(self.pos.y)-6, 12, 12), 0, border_radius=3)
...
class SlowTower(Tower):
    ...
    def draw(self, screen, selected=False):
        super().draw(screen, selected)
        pygame.draw.circle(screen, (120, 170, 255), (int(self.pos.x), int(self.pos.y)), 6, 2)
